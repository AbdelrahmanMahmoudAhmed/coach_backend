...
const addBlog = controllerWrapper(async (req, res, next) => {
    ...
    const requestedCertification = { titleAr, titleEn, contentAr, contentEn, type, link }
    type == "pic" && image ? (requestedCertification.image = image) : requestedCertification.image = null;
...
const deleteBlog = controllerWrapper(async (req, res, next) => {
    ...
    const data = await Blog.findOne({ where: { id } });
    if (!data) throw createAppError("This Blogs was not found", HttpStatus.NotFound, 100);
    // to delete the image when the blog item
    if (data.dataValues.image) {
        const filePath = path.join(__dirname, "..", "..", "..", "uploads", "blog", data.dataValues.image)
        clearImage(filePath)
    }
...
const updateBlog = controllerWrapper(async (req, res, next) => {
    ...
    if (!data) throw createAppError("This Blog was not found", HttpStatus.NotFound, 1);


    // to delete the image if there is a new one 
    if ((req.file?.filename && data.dataValues.image) || ((type == 'video') && data.dataValues.image)) { // to delete the old image to replace it with the new one
        const filePath = path.join(__dirname, "..", "..", "..", "uploads", "blog", data.dataValues.image)  
        clearImage(filePath) 
    }
